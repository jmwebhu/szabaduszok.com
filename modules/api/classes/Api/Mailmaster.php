...
{	
	...
	/**
	 * Feliratja a projekt tulajdonost a configban levo listara
	 * 
	 * @param Model_User $user		Felhasznalo
	 * @return mixed				API hivas eredmenye. Siker eseten mm_id, egyebkent self hibakodok
	 * 
	...
	public function subscribeProjectowner(Model_User $user)
	{
		try
		{
			...
			if (Kohana::$environment == Kohana::PRODUCTION)
			{
				...
				if ($result === false || in_array($result, [self::DUPLICATE_EMAIL_ERROR, self::EMAIL_SYNTAX_ERROR, self::UNKNOWN_ERROR]))
				{
					throw new Exception('Mailmaster API Projekt tulajdonos feliratkozas. Hiba kód: ' . $result);
				}
			}
		}
		catch (Exception $ex)
	...
	/**
	 * Modositja a projekt tulajdonost a configban levo listam
	 *
	 * @param Model_User $user		Felhasznalo
	 * @return mixed				API hivas eredmenye. Siker eseten mm_id, egyebkent self hibakodok
	 *
	...
	public function updateProjectowner(Model_User $user)
	{
		...
		{		
			...
			if (Kohana::$environment == Kohana::DEVELOPMENT)
			{
				...
				if ($result === false || in_array($result, [self::DUPLICATE_EMAIL_ERROR, self::EMAIL_SYNTAX_ERROR, self::UNKNOWN_ERROR]))
				{
					throw new Exception('Mailmaster API Projekt tulajdonos adatmódosítás. Hiba kód: ' . $result);
				}
			}
		}
		catch (Exception $ex)
	 ...
	 */
	protected function sendPostRequest($url, $data, $method = 'POST')
	{ 		
		$options = [
			'http' => [
