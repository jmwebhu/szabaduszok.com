...
class Kohana_ORM extends Model implements serializable {
	 ...
	 * @return ORM
	 */
	public function add($alias, $far_keys, $extra = null)
	{
		$far_keys = ($far_keys instanceof ORM) ? $far_keys->pk() : $far_keys;
		...
		foreach ( (array) $far_keys as $key)
		{
			if ($extra)
			{
				$query->values(array($foreign_key, $key, Arr::get($extra, 'value')));
			}
			else
			{
	...
	public function getAllWhere(array $where)
	{
		...
		foreach ($models as $model)
		{
			 ...
			 * @var $model ORM
			 */
			if ($model->{Arr::get($where, 'col')} == Arr::get($where, 'value'))
			{
				$result[] = $model;
			}
		}
    ...
    {
        return AB::select()->from($this)->where('slug', '=', $slug)->execute()->current();
    }
