...
class Kohana_Database_MySQL extends Database {
	...
	public function connect()
	{
		...
		try
		{
			...
			else
			{
				...
				/*echo Debug::vars($hostname);
				echo Debug::vars($username);
				echo Debug::vars($password);
				echo Debug::vars($database);
				exit;*/
	...
	public function list_tables($like = NULL)
	{
		if (is_string($like))
		{
			...
			$result = $this->query(Database::SELECT, 'SHOW TABLES LIKE '.$this->quote($like), FALSE)->cached();
		}
		else
		{
			// Find all table names
			$result = $this->query(Database::SELECT, 'SHOW TABLES', FALSE)->cached();
		}
	...
	public function list_columns($table, $like = NULL, $add_prefix = TRUE)
	{
		...
		if (is_string($like))
		{
			...
			$result = $this->query(Database::SELECT, 'SHOW FULL COLUMNS FROM '.$table.' LIKE '.$this->quote($like), FALSE)->cached();
		}
		else
		{
			// Find all column names
			$result = $this->query(Database::SELECT, 'SHOW FULL COLUMNS FROM '.$table, FALSE)->cached();
		}
